from libs.bab_scheme import BabPartialSolution, BabSolver, Solution
...
def binpacking_solve(objects: List[int], capacity: int):
    class BinPackingBabPS(BabPartialSolution):
        ...
        def calc_opt_bound(self) -> Union[int, float]:
            ...
            if self.container_weights:
                maximoEspacioLibre = max(capacity - self.container_weights[i] for i in range(len(self.container_weights)))
            else:
                maximoEspacioLibre = 0

            pesoTotalObjetos = 0
            for i in range(self.n, len(objects)):
                if objects[i] <= maximoEspacioLibre:        # Si me cabe ese objeto
                    pesoTotalObjetos += objects[i]          # Sumo el peso

            objetoMasPequeño = objects[-1]                  # coomo me los dan ordenados, es el ultimo

            libre = sum(capacity - w for w in self.container_weights if (w + objetoMasPequeño) <= capacity)

            dentro = min(libre, pesoTotalObjetos)

            quedaPorMeter = sum(objects[i] for i in range(self.n, len(objects))) - dentro

            return len(self.container_weights) + quedaPorMeter / capacity  # AHORA ES DEMASIADO OPTIMISTA
